...
for($i=0;$i<count($MqVtaUsada);$i++) {
	...
	$imgWS = base64_decode($cadenaWS);
	//escribimos la información obtenida en un archivo llamado 
	//unodepiera.png para que se cree la imagen correctamente
	file_put_contents('unodepiera'.$i.'.png', $imgWS);

	echo '<a id="'.$MqVtaUsada[$i]->Id_Maquinaria.'" class="VincProd">';
	echo '<div class="img-rounded producto" style="background: url(unodepiera'.$i.'.png) center no-repeat / cover;"></div>';
 
    echo '<div class="img-rounded"><label class="text-center">'.$MqVtaUsada[$i]->TipoMaquinaria.'-'.$MqVtaUsada[$i]->Marca.'-'.$MqVtaUsada[$i]->Modelo.'</label>';
    echo '</div></a>';
